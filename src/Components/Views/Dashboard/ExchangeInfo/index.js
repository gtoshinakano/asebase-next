...
const ExchangeInfo = () => {
  ...
  const hasChanged = !_.isEqual(_.map(form, i=> _.pick(i, toPick)), _.map(exchange?.data, i=>_.pick(i, toPick)))
  ...
  const isAwaiting = exchange.isLoading || exchange.isFetching || mutation.isLoading || mutation.isFetching || isMutatingExchange

  return (
    <>
      <h2 className="font-semibold inline-flex align-middle pt-7 pl-2 pb-4">
        <i className="ri-flight-takeoff-line mr-2 text-2xl"></i> <span className="my-auto text-xl">Bolsas no Japão</span>
      </h2>
      <div className="flex flex-wrap px-1 w-full">
        {form.length > 0 
          ...
          : <button 
              ...
              onClick={onInitForm}
            >
            <i className="ri-edit-2-fill mr-3 text-sky-400 my-auto text-3xl md:text-base"></i> Preencher informações sobre intercâmbios no Japão
          </button>
        }
      </div>
      <Transition
        ...
        <div className="pr-4 mt-3 w-full space-x-2 flex justify-end">
          {hasChanged &&
            ...
            <RoundButton 
              ...
              tooltip={hasError ? "Preencha os campos corretamente para poder salvar esta seção" : ""}
            >
