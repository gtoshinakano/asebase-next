...
const ProfessionalProfile = () => {
  ...
  const professional = useQuery(queryKey, getProfessionalProfile, {staleTime: Infinity})
  ...
  const isAwaiting = professional.isLoading || professional.isFetching || mutation.isLoading || mutation.isFetching || isMutatingProfessional
      ...
      <div className="flex flex-wrap px-1 w-full">
        {form.length > 0 
          ...
          : <button 
              ...
              onClick={onInitForm}
            >
            <i className="ri-edit-2-fill mr-3 text-sky-400 my-auto text-3xl md:text-base"></i> Preencher informações profissionais
          </button>
        }
      </div>
      <Transition
...
const toPick = ["start_year", "position", "end_year", "company_name", "current_job"]
